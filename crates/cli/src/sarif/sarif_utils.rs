...
mod tests {
    ...
    /// The SARIF `region.startColumn` / `region.endColumn` fields carry whatever value is in
    /// `Position.col`.  When the kernel emits UTF-16 columns (as it now does), the SARIF output
    /// automatically conforms to the SARIF v2.1 default encoding (UTF-16).
    ///
    /// This test constructs a violation whose col was computed as UTF-16 (e.g. after an emoji on
    /// the same line, col 3) and asserts the SARIF JSON contains that value.
    #[test]
    fn sarif_region_carries_utf16_col() {
        let rule = RuleBuilder::default()
            .name("utf16-col-rule".to_string())
            .description_base64(None)
            .language(Language::Python)
            ...
            .build()
            .unwrap();
        let sarif_rule = SarifRule::StaticAnalysis(Box::new(rule));

        // UTF-16 col 3 = position of an identifier after 🚀 on the same line.
        let violation = Violation {
            start: Position::new(1, 3),
            end: Position::new(1, 16),
            message: "utf16 col test".to_string(),
            severity: RuleSeverity::Error,
            category: RuleCategory::Security,
            fixes: vec![],
            taint_flow: None,
        ...
        let rule_result = RuleResultBuilder::default()
            .rule_name("utf16-col-rule".to_string())
            .filename("file.py".to_string())
            .violations(vec![violation])
            .errors(vec![])
            .execution_error(None)
            .output(None)
            .execution_time_ms(0u128)
            .parsing_time_ms(0u128)
            .query_node_time_ms(0u128)
            .build()
            .unwrap();
        let sarif_result = SarifRuleResult::try_from(rule_result).unwrap();
        ...
        .expect("generate sarif");

        let sarif_json = serde_json::to_value(&sarif).unwrap();
        let location =
            &sarif_json["runs"][0]["results"][0]["locations"][0]["physicalLocation"]["region"];
        assert_eq!(
            location["startColumn"], 3,
            "SARIF startColumn should be UTF-16 col 3"
        );
        assert_eq!(
            location["endColumn"], 16,
            "SARIF endColumn should be UTF-16 col 16"
        );
    }
}
