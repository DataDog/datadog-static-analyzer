...
impl JsRuntime {
        ...
        T: Fn(&mut v8::TryCatch<v8::HandleScope>, v8::Local<v8::Value>) -> U,
    {
        ...
        // If the watchdog requested termination, it should have marked `execution_complete` as true,
        // and so we don't need to set `execution_complete`, nor do we need to (inefficiently) re-notify via the condvar.
        // We just reset the v8 isolate's termination state and return a timeout error.
        if tc_ctx_scope.is_execution_terminating() {
            debug_assert!(self.watchdog_pair.0.lock().unwrap().execution_complete);
        ...
        loop {
            let mut state = cvar
                .wait_while(lock.lock().unwrap(), |state| {
                    state.execution_complete && !state.should_shut_down
                })
            ...
            if now >= timeout_threshold {
                ...
                // Trust that v8 will halt execution and eagerly set the state to complete so the
                ...
                let result = cvar
                    .wait_timeout_while(state, additional_wait, |state| !state.execution_complete)
                    .expect("mutex should not be poisoned");
                state = result.0;
...
"#;
    ...
    fn op_ts_node_named_children() {
        let mut rt = JsRuntime::try_new().unwrap();
        let source = "function echo(a, b, c) {}";
        let filename = "some_filename.js";
        let ts_query = r#"
...
"#;
        ...
        shorthand_execute_rule_internal(&mut rt, source, filename, ts_query, noop, None).unwrap();
        ...
        let expected = concat!(
            "[",
            r#"{"type":"identifier","start":{"line":1,"col":15},"end":{"line":1,"col":16},"text":"a"},"#,
            r#"{"type":"identifier","start":{"line":1,"col":18},"end":{"line":1,"col":19},"text":"b"},"#,
            r#"{"type":"identifier","start":{"line":1,"col":21},"end":{"line":1,"col":22},"text":"c"}"#,
            "]",
        );
        assert_eq!(console_lines[0], expected);
    }
